...
**BepInEx + MonoMod form:** if the patch project references `BepInEx/core/MonoMod.dll`, outputs to `BepInEx/monomod/`, or targets a game whose patches are applied by BepInEx's MonoMod.Loader at launch (rather than applied offline by you), read `references/bepinex-monomod-environment.md` **before** writing code. That reference covers: detecting this form, asking the user for the `BepInEx/` folder and extracting `MonoMod.dll` + the real target DLL from it, the PE32+ Mono target assembly that MSBuild drops with `MSB3246` (and the Cecil round-trip clean-reference fix), and the runtime `TypeLoadException` on injected types whose dependency DLLs live only in `monomod/` (not a runtime probe path). For that failure, an injected `AppDomain.AssemblyResolve` resolver is documented as an **optional recommended technique** — it yields single-folder (`monomod/`) release distribution, but you must **propose it to the user and wait for explicit opt-in** before implementing (do not inject it autonomously); a manual probe-path staging alternative is also documented. If the user opts into the resolver, mount it via an **explicit patch of the `TypeLoadException`'s top-frame method** calling `Register()` before `orig_` — **not** `[RuntimeInitializeOnLoadMethod]`, which does not fire on BepInEx-patched assemblies.
